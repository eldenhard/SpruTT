<template>
  <div>
    ...
    <button
      ...
      "
    >
      Запросить договора с покупателем
    </button>
    <br /><br />
      ...
      >
        ...
        <tbody>
          <tr v-for="buyer in buyerDirectory" :key="buyer.id">
            ...
            <td v-else>Нет</td>
            <td v-if="buyer.scan != null">
              <a :href="buyer.scan" target="_blank"
                ><img style="height: 50px" src="@/assets/pdf.png" alt="скан"
              /></a>
            </td>
            <td v-else>—</td>
            ...
            <td v-if="buyer.counterparty != null">
              {{ buyer.counterparty.work_name }}
            </td>
            <td v-else>—</td>

            <td v-if="buyer.counterparty != null">
              {{ buyer.counterparty.short_name }}
            </td>
            <td v-else>—</td>

            <td v-if="buyer.counterparty != null">
              {{ buyer.counterparty.full_name }}
            </td>
            <td v-else>—</td>

            <td v-if="buyer.counterparty != null">
              {{ buyer.counterparty.els }}
            </td>
            <td v-else>—</td>

            <td v-if="buyer.counterparty != null">
              {{ buyer.counterparty.ogrn }}
            </td>
            <td v-else>—</td>

            <td v-if="buyer.counterparty != null">
              {{ buyer.counterparty.inn }}
            </td>
            <td v-else>—</td>

            <td v-if="buyer.counterparty != null">
              {{ buyer.counterparty.kpp }}
            </td>
            <td v-else>—</td>

            <td v-if="buyer.counterparty != null">
              {{ buyer.counterparty.legal_address }}
            </td>
            <td v-else>—</td>

            <td
              style="font-size: 15px; text-align: center"
              v-if="buyer.counterparty != null"
            >
            ...
            <td v-if="buyer.counterparty != null">
              {{ buyer.counterparty.manager }}
            </td>
            <td v-else>—</td>

            <td v-if="buyer.counterparty != null">
              {{ buyer.counterparty.phone }}
            </td>
            <td v-else>—</td>

            <td v-if="buyer.counterparty != null">
              {{ getGroupName(buyer.counterparty.group) }}
            </td>
            <td v-else>—</td>
...
export default {
  ...
  methods: {
    ...
    getBuyerContract() {
      this.loader = true;
      api
        .getDirectoryBuyer(this.filter_buyer)
        .then((response) => {
