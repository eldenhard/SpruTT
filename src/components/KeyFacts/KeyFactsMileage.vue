<template>
  <div style="padding-bottom: -500px">
    ...
    <div class="hello1 m" ref="chartdiv3" v-show="month"></div>
    <div class="hello1 k" ref="chartdiv4" v-show="kvartal"></div>
  </div>
</template>
<style>
.hello1 {
...
export default {
  ...
  watch: {
    selectSearch() {
      if (this.selectSearch == 1) {
        this.day = true;
        (this.month = false), (this.kvartal = false), console.log("I WORK");
        this.getGraph();
      }
  ...
  mounted() {
    api.getKeyFacts().then((response) => {
      ...
      this.allData = this.Mileage.reduce(
        (acc, { day, speed, distance, wagons }) => {
      ...
      const AllKvartal = a.map(({ day, ...rest }) => {
        const quarter = ["I", "II", "III", "IV"][
          Math.floor((+day.substring(5, 7) - 1) / 3)
        ];
      ...
      this.Kvartal = AllKvartal.reduce(
        (acc, { hash, day, speed, distance, wagons }) => {
          const kvartal = hash;
          if (acc[kvartal]) {
